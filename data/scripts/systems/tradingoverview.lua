...
function refreshRoutesUI()
    ...
    for index = 1, 15 do
        ...
        for j = 1, 2 do
			...
			routePFrames[index]:hide()
			routeProfitLabel[index]:hide()
			routeAmountLabels[index][j]:hide()
			routeMaxCargoLabel[index]:hide()
        end
    end
    ...
    for i, route in pairs(routes) do
        ...
        if i > routesPage * 15 and i <= (routesPage + 1) * 15 then
            ...
            for j, offer in pairs({route.buyable, route.sellable}) do
				...
				if j == 1 then
					sCr = offer.price
					selling = math.floor(offer.stock)
					routeAmountLabels[index][j].caption = selling
				end
				if j == 2 then
					bCr = offer.price
					...
					local profit = 0
					profit = respectAmount * (bCr - sCr)
					routeProfitLabel[index].caption = createMonetaryString(profit) .. " Cr"%_t
					
					
					-- Calculating Max Cargo needed of Route
					local cargo = 0
					cargo = respectAmount * (round(offer.good.size, 2))
					routeMaxCargoLabel[index].caption = cargo
				...
				routePFrames[index]:show()
				routeProfitLabel[index]:show()
				routeAmountLabels[index][j]:show()
				routeMaxCargoLabel[index]:show()
...
function buildGui(window, guiType)
    ...
    local volX = 630
    local priceX = 700
    local priceFactorLabelX = 830
    local stationLabelX = 900
    local onShipLabelX = 1200
    local buttonX = 1480
...
function buildRoutesGui(window)
    ...
    for i = 1, 15 do
		...
		local psplit = UIVerticalSplitter(msplit.right, 10, 0, 0.5)
		psplit.rightSize = profitLabelX + cargoLabelX
		
		-- create UI for Profit & Cargo
		local px = psplit.right.lower.x
		...
		local profitLabel = window:createLabel(vec2(px + 10, yText), "", 15)
		local cargoLabel = window:createLabel(vec2(px + cargoLabelX, yText), "", 15)
		...
		profitLabel.font = "Arial"
		cargoLabel.font = "Arial"
			
		table.insert(routePFrames, pframe)
		table.insert(routeProfitLabel, profitLabel)
		table.insert(routeMaxCargoLabel, cargoLabel)
	

	
		
        local vsplit = UIVerticalSplitter(psplit.left, 10, 0, 0.5)
        ...
        for j, rect in pairs({vsplit.left, vsplit.right}) do
            ...
            if i == 1 then
                ...
                if j == 1 then
					...
					window:createLabel(vec2(x + amountLabelX, 10), "Sell Amount"%_t, 15)
                else
                    window:createLabel(vec2(x + stationLabelX, 10), "To"%_t, 15)
					window:createLabel(vec2(x + amountLabelX, 10), "Buy Amount"%_t, 15)
                end
            end
			...
			table.insert(routeAmountLabels[i], amountLabel)
