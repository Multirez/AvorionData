...
void main()
{
    ...
    float borderFactor = 1.0 - step(util0.x, texCoord.x) +
            step(util0.y, texCoord.x) +
            1.0 - step(util0.z, texCoord.y) +
            step(util0.w, texCoord.y);

    float factor = clamp(borderFactor + 1.0 - texOverlay.a, 0, 1);

    texColor.rgb = mix(texOverlay.rgb, texColor.rgb, factor);
    ...
    factor = clamp(borderFactor + 1.0 - normalOverlay.a, 0, 1);
    normalTex = mix(normalOverlay.xyz, normalTex, factor);
